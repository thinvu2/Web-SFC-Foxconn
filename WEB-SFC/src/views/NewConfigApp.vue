  ...
  <div class="content" style="min-height:700px">
    ...
    <div class="row">
      <template v-for="(item, index) in dataList1" :key="index">
        <div class="icon-app-disable col-sm-4" style="padding: 0 0px 8px 35px;border: 1px" v-if="item.IsDisable == 0">
          <div class="icon-content">
                <span class= "config-border font-text">[{{ item.Id }}] . </span>
                <span class="text-config-name font-text">{{ item.Name }}</span>
          </div>          
        </div>
        <div v-else class="icon-app col-sm-4" style="padding: 0 0px 8px 35px;cursor:pointer;border: 1px" @click="GotoRoute(item.Route)">
