    ...
    <div v-if="!isHideMenu">
      ...
      <div class="top-left div-top col-12 row">
        <div class="col-3 row select-div">
          ...
          >
            <option
              v-for="(item, index) in Model_Names"
              :key="index"
              :value="item"
            >
              {{ item }}
        ...
        <div class="col-3 row select-div">
          ...
          >
            ...
            <option
              v-for="(item, index) in Mo_Numbers"
              :key="index"
              :value="item"
            >
              {{ item }}
        ...
        <div class="col-3 row select-div">
          ...
          <span  class="text-title">CHOOSE</span>
          <select
            v-model="chooseField"
            name="select"
            class="form-control form-control-sm select-form"
          >          
            ...
            <option value="REPAIR_SN">REPAIR_SN</option>
            <option value="LOCATION">LOCATION</option>
            <option value="REPAIR_DAY">REPAIR_DAY</option>
            <option value="REPAIR_MO">REPAIR_MO</option>
            <option value="REPAIR_WIP">REPAIR_WIP</option>
          </select>
        </div>
        <div class="col-3 row select-div"> 
        </div>
      ...
      <div class="top-left div-top col-12 row">
        <div class="col row div-period">
          <input
            ...
            v-model="period"
          />
          <label for="checkboxPeriod" style="margin-top: 5px;"><b class="titleType">Period</b></label>
        </div>        
        <div class="col">
          <datepicker
            :disabled="!period"
...
export default {
  ...
  methods: {
    async GetModel() {
      ...
      var { data } = await Repository.getRepo("GetModelRepair", payload);
      data.data.forEach((element) => {
        this.Model_Names.push(element.MODEL_NAME);
      });
    },
    async GetMoRepair(){
      this.Mo_Numbers = [];
      ...
      var { data } = await Repository.getRepo("GetMoRepair", payload);
      data.data.forEach((element) => {
        this.Mo_Numbers.push(element.MO_NUMBER);
      });
    },
    ...
    async querySearch() {
      ...
      if ((this.valueSearch.length > 0 && this.chooseField != null) || 
          (this.chooseField == "REPAIR_WIP" && this.Model_name != "ALL")) {
        ...
        var payload = {
          ...
          input:  this.valueSearch,
          field: this.chooseField,
          date_from: date_from,
          date_to: date_to,
        };
        var { data } = await Repository.getRepo("QueryRepair", payload);
        ...
        if(this.listrate.length > 0){
          this.listrate.forEach((element) =>{
            this.repair_rate = "The "+ this.valueSearch + "'s repair_rate is "+element.RATE+"%";
          });
        }
