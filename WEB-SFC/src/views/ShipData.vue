  ...
  <div class="div-main" style="background-color: #4e7da6">
    ...
    <div v-if="!isHideMenu">
      ...
      <div class="top-left div-top col-12 row">
        <div class="col-3 row select-div">
          <!-- <span class="text-title">Type</span> -->
          <span  class="text-title">RECORD_TYPE</span>
          <select
            v-model="record_type"
            name="select"
            class="form-control form-control-sm select-form"
          >          
            <option value="WIPC">WIPC</option>
            <option value="SHIP">SHIP</option>
            <option value="BDSN">BDSN</option>
            <option value="SHPCFM">SHPCFM</option>
          </select>
        </div>
        <div class="col-3 row select-div">
          <!-- <span class="text-title">Type</span> -->
          <span  class="text-title">MODEL_TYPE</span>
          <select
            :disabled="byDN"
            v-model="model_type"
            name="select"
            class="form-control form-control-sm select-form"
            v-on:change="UpdateModel_type"
          >          
          <template v-for="(item, index) in listmodel_type" :key="index">
              <option :value="item">{{ item }}</option>
          </template>
          </select>
        </div>
        <div class="col-3 row select-div">
          <span class="text-title" for="validationDefault02">MODEL NAME</span>
          <DropdownSearch
            :disabled="byDN"
            class="form-control form-control-sm select-form" 
            :listAll="ListModel"
      ...
      <div class="top-left div-top col-12 row">
        <div class="col row div-period">
          <input
            v-on:change="UpdateByPeriod"  
            id="checkboxPeriod"
            style="margin-left: 20px;  margin-right: 8px"
            type="checkbox"
            v-model="period"
          />
          <label for="checkboxPeriod" style="margin-top: 18px;"><b class="titleType">By Creat_time</b></label>
        </div>        
        <div class="col">
          <datepicker
            :disabled="!period"
    ...
    <div class="col-12 ">
      ...
      <div class="div-searchbox">
        <div class="col div-period">
          <div class="col-2">
            <input          
              v-on:change="UpdateByDN"      
              id="checkboxDN"
              style="margin-right: 8px"
              type="checkbox"
              v-model="byDN"
            />
            <label for="checkboxDN" style="margin-top: 20px;"><b class="titleType">By DN</b></label>
          </div> 
          <div class="col-2">
            <input          
              v-on:change="UpdateByTelit"      
              id="isTelit"
              style="margin-right: 8px"
              type="checkbox"
              v-model="isTelit"
            />
            <label for="isTelit" style="margin-top: 20px;"><b class="titleType">By FTI DN</b></label>
          </div> 
        </div> 
  ...
  <div class="div-content-bellow">
    <div class="div-export row" v-if="qmDataTable.length > 0">
      <div>
        <span
          ><b class="text-count">
            {{ $store.state.language == "En" ? "Show wip:" : "Hi?n th? wip" }}
            <span class="count-number">{{ qmDataTable.length }}</span>
            {{ $store.state.language == "En" ? " of " : " c?a " }}
            <span class="count-number">{{ allDataTable.length }}</span>
            {{ $store.state.language == "En" ? "records" : "b?n ghi" }}</b
          ></span
        >
        <p class="text-count">{{ repair_rate }}
          </p>
      </div>
    ...
    <template v-if="subDataTable">
      <div class="div-export row" v-if="subDataTable.length > 0">
        <div>
          <span
            ><b class="text-count">
              {{ $store.state.language == "En" ? "Show:" : "Hi?n th?" }}
...
import dataQInv2 from "../data/menu_qInv2";
...
export default {
  ...
  created() {
    ...
    this.Model_Names.push("ALL");    
    this.GetModel_type();
    this.GetModel("NIC");
    this.menuInv2 = dataQInv2;
  ...
  methods: {
    UpdateModelNameReceive(event) {
      this.MODEL_NAME = event;     
    },  
 
    UpdateModel_type(event) {
      this.MODEL_NAME = "ALL";
      this.ListModel = [];
    ...
    async GetModel(value) {
      ...
      var { data } = await Repository.getRepo("GetCustPN", payload);
      this.ListModel.push("ALL")
    ...
    async GetModel_type() {
      ...
      var { data } = await Repository.getRepo("GetModel_type", payload);

      data.data.forEach((element) => {
        this.listmodel_type.push(element.GROUPCODE);
      });
    },
    ...
    exportexcelxlsx(){
       ...
       //if(typeof xlsx == 'undefined') xlsx = require('xlsx');
        ...
        /* make the worksheet */
        var ws = xlsx.utils.json_to_sheet(this.allDataTable);
    ...
    async querySearch() {
      this.valueSearch=this.valueSearch.replace(/[\u200B-\u200D\uFEFF]/g,"");
        console.log(this.byDN);
        console.log(this.isTelit);
      if(!this.isTelit){
       if((this.byDN && this.period )||(!this.byDN && !this.period )){
            if (localStorage.language == "En") {
              this.$swal("", "ByDN or By Creat_time check!", "error");
            } else {
              this.$swal("", "Ch?n ByDN ho?c ByCreat_time", "error");
            } 
            return;  
        }
        ...
        if(this.byDN == true){
          if (this.valueSearch.trim() == ""){
            if (localStorage.language == "En") {
              this.$swal("", "ByDN check! Not value input empty", "error");
            } else {
              this.$swal("", "Không du?c d? tr?ng nh?p DN", "error");
            } 
            return;  
          }
        }
        ...
        var { data } = await Repository.getRepo("GetShippingData", payload);
        console.log(data);
        if(data.result == "ok"){
          this.selectAll();
          var $elem = $(".div-table");
          ...
          if (localStorage.language == "En") {
            this.$swal("", "Not exist data", "error");
          } else {
            this.$swal("", "Không t?n t?i d? li?u", "error");
          } 
        }
      }else{
        if(this.valueSearch == "" || this.valueSearch == null){
            if (localStorage.language == "En") {
              this.$swal("", "Input DN not empty!", "error");
            } else {
              this.$swal("", "Không du?c d? input r?ng", "error");
            } 
            return;  
        }
        var payload1 = {
          database_name: localStorage.databaseName,
          FTI_DN:  this.valueSearch,
        };
        var  response  = await Repository.getRepo("GetShippingTelit", payload1);
        ...
        if(data1.result == "ok"){
          this.selectAll();
          var $elem1 = $(".div-table");
          this.allDataTable = data1.data;
          //this.subDataTable = data1.data;
          this.listrate = data1.rate;
