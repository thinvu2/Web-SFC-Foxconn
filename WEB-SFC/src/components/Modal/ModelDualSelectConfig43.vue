<template>
  <div class="myModal">
    <transition name="fade" appear>
      <div
        class="modal-overlay"
        v-if="$store.state.isShowConfig43"
        @click="HideModal()"
      ></div>
    </transition>
      ...
      >
        ...
        >
          <label class="label-head" for="textSearch">{{
            $store.state.language == "En" ? "Search group" : "Tìm ki?m group"
          }}</label>
        ...
        <div class="content-modal">
          ...
          <div class="row div-title col-12">
            <div class="col-5">
              <b>{{
                $store.state.language == "En" ? "Selected" : "Ðã ch?n"
              }}</b>
            </div>
            <div class="col-2"></div>
            <div class="col-5">
              <b>{{ $store.state.language == "En" ? "Avaiable" : "Có s?n" }}</b>
            </div>
          </div>
            ...
            <div class="col-5 content-bellow">
              <smooth-scrollbar>
                <ul class="select-model">
                  <li
                    @click="MoveAtoB(item)"
                    v-for="(item, index) in $store.state.listSelectDualConfig43"
                    :key="index"
                  >
                    {{ item.VALUE }}
                  </li>
  ...
  methods: {
    ...
    searchKeyWord() {
      if (this.textSearch.trim().length == 0) {
        this.isHideClearButton = true;
        this.$store.commit("Config43ListSearchEqualAvaiGroup");
      } else {
    ...
    MoveAllBToA() {
      if (!this.$store.state.isSelectOne) {
        this.$store.commit("moveAllListAvaiToSelectConfig43");
      }
    },
    MoveAllAToB() {
      this.$store.commit("moveAllListSelectToAvaiConfig43");
    },
    MoveAtoB(item) {
      this.$store.commit("moveItemToAvaiConfig43", item);
    },
    MoveBtoA(item) {
      if (this.$store.state.isSelectOne) {
        if (this.$store.state.listSelectDualConfig43.length == 0) {
          this.$store.commit("moveItemToSelectConfig43", item);
        }
      } else {
        this.$store.commit("moveItemToSelectConfig43", item);
      }
