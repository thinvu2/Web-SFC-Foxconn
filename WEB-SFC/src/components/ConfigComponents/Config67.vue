...
<!--  
        <div class="col-md-3 mb-3">        
          ...
          <input
            ...
            type="number"
            :textContent="model.WEIGHT" 
            :value="model.WEIGHT"
            textPlaceHolder="Enter WEIGHT"
            list="Vercode"
            v-on:change="UpdateWeightReceive" 
          />
        ...
        <div class="col-md-3 mb-3">
          <label for="STANDARD_WEIGHT">STANDARD_WEIGHT(g)</label>
          <input
            class="form-control form-control-sm text-element"
            id="STANDARD_WEIGHT"
            :value="model.STANDARD_WEIGHT"
            type="number"
            @input="updateSTANDARD_WEIGHT"
            textPlaceHolder="Enter STANDARD_WEIGHT"  />
        </div>

 
        <div class="col-md-3 mb-3">        
          <label for="validationDefault02">TOLERANCE</label>
          <input
            class="form-control form-control-sm " 
            type="number"
            :textContent="model.WEIGHT" 
...
export default {
  ...
  methods: {
    ...
    UpdateWeightReceive: function(event) {
      this.model.WEIGHT = event.target.value;
    },  
    updateSTANDARD_WEIGHT(event) {
    ...
    async GetModel() {
      ...
      var { data } = await Repository.getRepo("Config67_MODEL", payload);
    ...
    async SaveData() {
      if (this.model.MODEL_NAME == "" || this.model.STANDARD_WEIGHT == "" || this.model.WEIGHT == "" ) {
        ...
        this.$swal({
          ...
          var payload = {
            ...
            STANDARD_WEIGHT: this.model.STANDARD_WEIGHT,      
          };

          var { data } = await Repository.getRepo("InsertUpdateConfig67",payload);
    ...
    DeleteRecord(item) {
      ...
      this.$swal({
        ...
        var { data } = await Repository.getRepo("DeleteConfig67", payload);
        if (data.result == "ok") {
          await this.QuerySearch();
          if (localStorage.language == "En") {
            this.$swal("", "Apply successfully", "success");
    ...
    ShowDetail(detail) {
        ...
        this.model.ID = detail.ROWID;
        this.model.MODEL_NAME = detail.MODEL_NAME;
        this.model.WEIGHT = detail.WEIGHT;
    ...
    async LoadComponent() {
      ...
      var { data } = await Repository.getRepo("GetConfig67Content", payload);
      this.DataTable = [];
