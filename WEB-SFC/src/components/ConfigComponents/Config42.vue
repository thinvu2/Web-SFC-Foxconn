  ...
  <div class="div-all">
    <div class="row">
      ...
      <div class="div-config-name row">
        <span>SAP GROUP MAPPING (42):</span>
      </div>
    </div>
    ...
    <div class="div-bellow">
      <div class="form-row">
        <div class="col-md-3 mb-3"> 
          <label for="validationDefault02">ROUTE_TYPE</label>
          <DropdownSearch
            class="form-control form-control-sm text-element col-md-3"
            :listAll="ListRouteType"
            @update-selected-item="UpdateRouteReceive"
            :textContent="model.ROUTE_TYPE"
            type="model"
            textPlaceHolder="Enter route type"
          />
        </div>
        ...
        <div class="col-md-3 mb-3" >
          <label for="validationDefault02"> SEQUENCE_NO</label>
          <input
            type="number"
            class="form-control form-control-sm text-element"
            id="validationDefault02"
...
export default {
  ...
  methods: {
    ...
    UpdateRouteReceive(value) {
      this.model.ROUTE_TYPE = value;
    ...
    async GetModel() {
      ...
      var { data } = await Repository.getRepo("GetallModel_CF42", payload);
    ...
    async GetAllGroup() {
      ...
      var { data } = await Repository.getRepo("GetallGroupCF42", payload);
      data.data.forEach((element) => {
        this.ListGroup.push(element.GROUP_NAME);
      });
     
    },

    async GetAllRoutetype() {
      var payload = {
        database_name: localStorage.databaseName,
      };
      var { data } = await Repository.getRepo("GetallRouteTypeCF42", payload);
      data.data.forEach((element) => {
        this.ListRouteType.push(element.ROUTE_TYPE);
      });
    ...
    async SaveData() {
      if (this.model.MODEL_NAME == "" || this.model.GROUP_NAME == "" || this.model.SEQUENCE_NO == ""
      || this.model.ROUTE_TYPE == ""
      ) {
    ...
    DeleteRecord(item) {
      ...
      this.$swal({
        ...
        var { data } = await Repository.getRepo("InsertUpdateConfig42", payload);
        if (data.result == "ok") {
          await this.QuerySearch();
          if (localStorage.language == "En") {
            this.$swal("", "Apply successfully", "success");
    ...
    ShowDetail(detail) {
        ...
        this.model.SEQUENCE_NO = detail.SEQUENCE_NO;
        this.model.ROUTE_TYPE= detail.ROUTE_TYPE;
    ...
    async LoadComponent() {
      ...
      var { data } = await Repository.getRepo("GetConfig42Content", payload);
      this.DataTable = [];
