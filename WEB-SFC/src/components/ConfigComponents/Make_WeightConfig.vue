...
export default {
  ...
  methods: {
    ...
    async SaveData() {
      ...
      if (this.model.MODEL_NAME == "") {
        ...
        this.$swal({
          ...
          var { data } = await Repository.getRepo("InsertUpdateMake_weightConfig",this.model);
    ...
    async LoadComponent() {
      ...
      var { data } = await Repository.getRepo("GetConfigMake_WeightContent", payload);
      this.DataTable = [];
    ...
    DeleteRecord(item) {
      ...
      this.$swal({
        ...
        var { data } = await Repository.getRepo("DeleteMake_weightConfig", payload);
        if (data.result == "ok") {
          await this.QuerySearch();
          if (localStorage.language == "En") {
            this.$swal("", "Apply successfully", "success");
