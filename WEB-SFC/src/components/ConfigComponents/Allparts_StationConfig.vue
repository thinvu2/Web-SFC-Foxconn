  ...
  <div class="div-all">
    <div class="row">
      ...
      <div class="div-config-name row">
        <span>Allparts_StationConfig:</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async GetGroupConfig7( listType, modalType) {
      ...
      var payload = {
        database_name: localStorage.databaseName,
        MODEL_NAME: this.textSearch,
      };
      var { data } = await Repository.getRepo("getConfigAllparts_Station", payload);
      this.$store.commit(listType, data.list);
      this.$store.commit(modalType);
    },
    UpdateGROUP_NAME: function(event) {
      this.model.GROUP_NAME = event.target.value;
    },
    ...
    async GetModel() {
      ...
      var { data } = await Repository.getRepo("ConfigAllpartsGetAllModel", payload);
    ...
    async GetVersion(value) {
      ...
      var { data } = await Repository.getRepo("ConfigLabel3sGetVersion", payload);
      if(this.model.MODEL_NAME != ""){
        this.ListVersion = [];  
        data.data.forEach((element) => {
          this.ListVersion.push(element.VERSION);
        });
      }
    },
    ...
    async SaveData() {
      if (this.model.MODEL_NAME == ""  || this.$store.state.listSelectDualModel == null) {        
        ...
        this.$swal({
          ...
          var { data } = await Repository.getRepo(
            "InsertUpdateConfigAllparts_Station",
            payload
          );
          ...
          }else if (data.result == "group") {
            if (localStorage.language == "En") {
              this.$swal("", "Not empty GROUP", "error");
            } else {
              this.$swal("", "Không để trông GROUP", "error");
            }
          } else {
    ...
    DeleteRecord(item) {
      ...
      this.$swal({
        ...
        var { data } = await Repository.getRepo("DeleteAllparts_Station", payload);
        if (data.result == "ok") {
          await this.QuerySearch();
          if (localStorage.language == "En") {
            this.$swal("", "Apply successfully", "success");
    ...
    ShowDetail(detail) {
        ...
        //this.CheckPriLabel3s("CONFIRM_NIC3S",detail)   
        this.GetGroupOfEmNo(detail.MODEL_NAME);
    },
    async GetGroupOfEmNo(model){
        var payload={
            database_name: localStorage.databaseName,
            MODEL_NAME:model,
        }
        ...
        if(data.result == "ok"){
          this.$store.state.listSelectDualModel=data.list1;
    ...
    async CheckPriLabel3s(value,model) {
      var payload = {
        database_name: localStorage.databaseName,
