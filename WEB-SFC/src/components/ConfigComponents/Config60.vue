    ...
    <div class="div-bellow">
      <div class="form-row">
        <div class="col-md-9 mb-3">
          <label for="validationDefault02">SHIP ADDRESS</label>
          <input
            type="text"
            class="form-control form-control-sm text-element"
            id="validationDefault02"
            v-model="model.SHIP_ADDRESS"
            
            required
          />
        </div>
        <div class="col-md-3 mb-3">
          <label for="validationDefault02">SHIP CODE</label>
          <input
            type="text"
            class="form-control form-control-sm text-element"
            id="validationDefault02"
            v-model="model.SHIP_CODE"
...
export default {
  ...
  methods: {
    UpdateShipIndexReceive(value) {
   
      this.model.SHIP_INDEX = value;
    },
    async GetShip_index() {
      var payload = {
        database_name: localStorage.databaseName,
      };
      var { data } = await Repository.getRepo("GetShip_index", payload);
      data.data.forEach((element) => {
        this.ListModel.push(element.SHIP_INDEX);
      });
    },
    ...
    async SaveData() {
      if ( this.model.SHIP_ADDRESS == "" || this.model.SHIP_CODE == ""
      ) {
    ...
    DeleteRecord(item) {
      ...
      this.$swal({
        ...
        var payload = {
          ...
          SHIP_INDEX: item.SHIP_INDEX,
          SHIP_ADDRESS: item.SHIP_ADDRESS,
          SHIP_CODE : item.SHIP_CODE,
          ACTION_TYPE : 'DELETE'
        };
        var { data } = await Repository.getRepo("InsertUpdate60", payload);
        if (data.result == "ok") {
          await this.QuerySearch();
          if (localStorage.language == "En") {
            this.$swal("", "Apply successfully", "success");
    ...
    ClearForm() {
     ...
     this.model.SHIP_ADDRESS ='';
     this.model.SHIP_CODE= '';
     this.model.ACTION_TYPE='INSERT';
    },
    ShowDetail(detail) {
      this.isReadonly= true;
      this.show = true;
       this.model.SHIP_INDEX= detail.SHIP_INDEX ;
       this.model.SHIP_ADDRESS= detail.SHIP_ADDRESS;
       this.model.SHIP_CODE= detail.SHIP_CODE;
    ...
    async LoadComponent() {
      ...
      var payload = {
        database_name: localStorage.databaseName,
        SHIP_INDEX: this.valueSearch,
      };
      var { data } = await Repository.getRepo("GetConfig60Content", payload);
      this.DataTable = [];
