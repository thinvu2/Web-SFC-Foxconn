      ...
      >
        ...
        >
          <label class="label-head" for="textSearch">{{
            $store.state.language == "En" ? "Search MO" : "Tìm kiếm MO"
          }}</label>
  ...
  methods: {
    ...
    async searchKeyWord() {
      if (this.textSearch.trim().length != 0) {
        ...
        var payload = {
          ...
          type: "mo",
          mo_number: this.textSearch.trim(),
    ...
    MoveAllBToA() {
      if (!this.$store.state.isSelectOne) {
        this.$store.commit("moveAllListAvaiToSelectMO");
      }
    },
    MoveAllAToB() {
      this.$store.commit("moveAllListSelectToAvaiMO");
    },
    MoveAtoB(item) {
      this.$store.commit("moveItemToAvaiMO", item);
    },
    MoveBtoA(item) {
      if (this.$store.state.isSelectOne) {
        if (this.$store.state.listSelectDualMO.length == 0) {
          this.$store.commit("moveItemToSelectMO", item);
        } else {
          this.$store.commit("moveAllListSelectToAvaiMO");
          this.$store.commit("moveItemToSelectMO", item);
        }
