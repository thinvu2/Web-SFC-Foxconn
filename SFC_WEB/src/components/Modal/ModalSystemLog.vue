<template>
  <div class="app">
    <transition name="fade" appear>
      <div
        class="modal-overlay"
        v-if="$store.state.isShowSystemLog"
        @click="hideModal()"
      ></div>
    </transition>
    <transition name="slide" appear>
      <div class="my_modal" v-if="$store.state.isShowSystemLog">
        ...
        <div class="content-top col-sm-12">
          ...
          >
            ...
            <option value="VSC2000">OBA PTM File VSC2000</option>
          </select>
          <button
    ...
    async SearchList() {
      if (this.textListSearch.length != 0) {
        ...
        this.textListSearch.split(/\n/).forEach((element) => {
          ...
          listInput.push({ input: element.trim() });
        });
