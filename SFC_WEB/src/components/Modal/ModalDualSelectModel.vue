<template>
  <div class="app">
    <transition name="fade" appear>
      <div
        class="modal-overlay"
        v-if="$store.state.isShowQMModel"
  ...
  methods: {
    ...
    searchKeyWord() {
      if (this.textSearch.trim().length == 0) {
        this.isHideClearButton = true;
        this.$store.commit("QMListSearchEqualAvaiModel");
      } else {
    ...
    MoveAllBToA() {
      this.$store.commit("moveAllListAvaiToSelectModel");
    },
    MoveAllAToB() {
      this.$store.commit("moveAllListSelectToAvaiModel");
    },
    MoveAtoB(item) {
      this.$store.commit("moveItemToAvaiModel", item);
    },
    MoveBtoA(item) {
      this.$store.commit("moveItemToSelectModel", item);
    },
  },
};
</script>
