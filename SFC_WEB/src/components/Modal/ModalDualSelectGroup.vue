<template>
  <div class="myModal">
    <transition name="fade" appear>
      <div
        class="modal-overlay"
        v-if="$store.state.isShowQMGroup"
        @click="HideModal()"
      ></div>
    </transition>
      ...
      >
        ...
        >
          <label class="label-head" for="textSearch">{{
            $store.state.language == "En" ? "Search group" : "Tìm kiếm group"
          }}</label>
  ...
  methods: {
    ...
    searchKeyWord() {
      if (this.textSearch.trim().length == 0) {
        this.isHideClearButton = true;
        this.$store.commit("QMListSearchEqualAvaiGroup");
      } else {
    ...
    MoveAllBToA() {
      if (!this.$store.state.isSelectOne) {
        this.$store.commit("moveAllListAvaiToSelectGroup");
      }
    },
    MoveAllAToB() {
      this.$store.commit("moveAllListSelectToAvaiGroup");
    },
    MoveAtoB(item) {
      this.$store.commit("moveItemToAvaiGroup", item);
    },
    MoveBtoA(item) {
      if (this.$store.state.isSelectOne) {
        if (this.$store.state.listSelectDualGroup.length == 0) {
          this.$store.commit("moveItemToSelectGroup", item);
        }
      } else {
        this.$store.commit("moveItemToSelectGroup", item);
      }
