<template>
  <div class="app">
    <transition name="fade" appear>
      <div
        class="modal-overlay"
        v-if="$store.state.isShowModalConfig7"
      ...
      >
        ...
        >
          <label class="label-head" for="textSearch">{{
            $store.state.language == "En" ? "Search model" : "Tìm kiếm model"
          }}</label>
        ...
        <div class="content-modal">
          ...
          <div class="row div-title col-12">
            ...
            <div class="col-5">
              <b>{{
                $store.state.language == "En" ? "Available" : "Có sẵn"
              }}</b>
            </div>
          </div>
            ...
            <div class="col-5 content-bellow">
              <smooth-scrollbar>
                <ul class="select-model">
                  <li
                    @click="MoveAtoB(item)"
                    v-for="(item, index) in $store.state.listSelectDualModelConfig7"
                    :key="index"
                  >
                    {{ item.VALUE }}
                  </li>
  ...
  methods: {
    ...
    searchKeyWord() {
      if (this.textSearch.trim().length == 0) {
        this.isHideClearButton = true;
        this.$store.commit("Config7ListSearchEqualAvaiGroup");
      } else {
    ...
    MoveAllBToA() {
      this.$store.commit("moveAllListAvaiToSelectGroupConfig7");
    },
    MoveAllAToB() {
      this.$store.commit("moveAllListSelectToAvaiGroupConfig7");
    },
    MoveAtoB(item) {
      this.$store.commit("moveItemToAvaiGroupConfig7", item);
    },
    MoveBtoA(item) {
      this.$store.commit("moveItemToSelectGroupConfig7", item);
    },
  },
};
</script>
