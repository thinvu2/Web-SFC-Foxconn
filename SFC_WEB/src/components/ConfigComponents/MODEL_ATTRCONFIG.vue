    ...
    <div class="div-bellow">
      <div class="form-row">
        <div class="col-md-3 mb-3">
          <label for="validationDefault02">VR_NAME</label>
          <DropdownSearch
            class="form-control form-control-sm text-element col-md-3"
            :listAll="ListVR_NAME"
            @update-selected-item="UpdateVR_NAMEReceive"
            :textContent="model.VR_NAME"
            type="model"
            textPlaceHolder="Enter VR_NAME"
          />
        </div>
        <div class="col-md-3 mb-3">
        ...
        <div class="col-md-3 mb-3">
              ...
              :textContent="model.VERSION" 
              :value="model.VERSION"
              textPlaceHolder="Enter VERSION"    
              v-on:change="UpdateVERSION"  
          />
        </div>
        <div class="col-md-3 mb-3">
              <label for="validationDefault02">VR_VALUE</label>
              <input type="text" class="form-control form-control-sm " 
              :textContent="model.VR_VALUE" 
              :value="model.VR_VALUE"
              textPlaceHolder="Enter VR_VALUE"    
              v-on:change="UpdateVR_VALUE"  
          />
        </div>
        <div class="col-md-3 mb-3">
           <label for="validationDefault02">DATA1</label>
              <input type="text" class="form-control form-control-sm " 
              :textContent="model.DATA1" 
              :value="model.DATA1"
              textPlaceHolder="Enter DATA1"    
              v-on:change="UpdateDATA1"      
              >
        </div>
        <div class="col-md-3 mb-3" >
           <label for="validationDefault02">DATA2</label>
              <input type="text" class="form-control form-control-sm " 
              :textContent="model.DATA2" 
              :value="model.DATA2"
              textPlaceHolder="Enter DATA1"    
              v-on:change="UpdateDATA2"   
              >
        </div>
...
export default {
  ...
  mounted() {
    document.title = "MODEL_ATTRCONFIG";
    this.CheckPrivilege();
    this.GetModel();
    this.GetATT();
  },
  methods: {
    ...
    UpdateVR_NAMEReceive(value) {
      this.model.ID = "";
      this.model.VR_NAME = value;
    ...
    UpdateVersionReceive: function(event) {
      this.model.VERSION = event.target.value;     
    },     
    GetStatusLabel(value){
      this.model.STATUS = value; 
    },    
    UpdateVR_VALUE: function(event) {
      this.model.VR_VALUE = event.target.value;
    },      
    UpdateVERSION: function(event) {
      this.model.VERSION = event.target.value;
    },
    UpdateDATA1: function(event) {
      this.model.DATA1 = event.target.value;
    },
    UpdateDATA2: function(event) {
    ...
    async GetModel() {
      ...
      var { data } = await Repository.getRepo("ConfigPACK_CTN_ISBRACKETModel", payload);
    ...
    async GetATT() {
      ...
      var { data } = await Repository.getRepo("GetATT", payload);
      this.VR_NAME = [];
      data.data.forEach((element) => {
        this.ListVR_NAME.push(element.OUTPUT);
      });
    },
    ...
    async SaveData() {
      if (this.model.MODEL_NAME == "" || this.model.VR_NAME == "" ) {
        ...
        this.$swal({
          ...
          var payload = {
              ...
              database_name: localStorage.databaseName,
              EMP: localStorage.username,
              VR_NAME: this.model.VR_NAME,
              MODEL_NAME: this.model.MODEL_NAME,
              VERSION: this.model.VERSION,
              VR_VALUE:this.model.VR_VALUE,
              DATA1:this.model.DATA1,
              DATA2:this.model.DATA2,
              listGroup:this.$store.state.listSelectDualModel,
          };
          var { data } = await Repository.getRepo("InsertUpdatePACK_CTN_ISBRACKET",payload);
          ...
          }else if (data.result == "notupdate") {
            await this.QuerySearch();
            if (localStorage.language == "En") {
              this.$swal("", "Data not exist! Not Update", "error");
            } else {
              this.$swal("", "Dữ liệu không tồn tại không thể sửa", "error");
            }
    ...
    ShowDetail(detail) {
        ...
        this.model.VERSION = detail.VERSION;
        this.model.VR_NAME = detail.VR_NAME;    
        this.model.VR_VALUE = detail.VR_VALUE;   
        this.model.DATA1 = detail.DATA1;   
        this.model.DATA2 = detail.DATA2;  
        if(detail.VR_NAME == "LABEL_QTY"){
    ...
    async CheckPrivilege() {
      ...
      if (data.result != "ok") {
        ...
        this.model.VR_NAME = data.output;
        this.LoadComponent();
      }
    },
    async LoadComponent() {
      ...
      var { data } = await Repository.getRepo("GetConfigPACK_CTN_ISBRACKETContent", payload);
      this.DataTable = [];
    ...
    DeleteRecord(item) {
      ...
      this.$swal({
        ...
        var { data } = await Repository.getRepo("DeletePACK_CTN_ISBRACKET", payload);
        if (data.result == "ok") {
          await this.QuerySearch();
          if (localStorage.language == "En") {
            this.$swal("", "Apply successfully", "success");
