    ...
    <div class="div-bellow">
      <div class="form-row">        
        <div class="col-md-4 mb-4">
          ...
          <input type="text" class="form-control text-element form-control-sm " 
          :value="model.MO_NUMBER"
          v-on:change="UpdateMO_NUMBER"  
        ...
        <!-- <div class="col-md-4 mb-4">
          ...
          <DropdownSearch
            ...
            @update-selected-item="UpdateMO_NUMBERReceive"
        ...
        <div class="col-md-4 mb-4">
          <label for="validationDefault02">ROUTE_NAME</label>
          <DropdownSearch
            class="form-control form-control-sm text-element col-md-3"
            :listAll="ListROUTE_NAME"
            @update-selected-item="UpdateROUTE_NAMEReceive"
        ...
        <div class="col-md-4 mb-4">
          <label for="validationDefault02">BOM_NO</label>
          <DropdownSearch
            class="form-control form-control-sm text-element col-md-3"
            :listAll="ListBOM_NO"
            @update-selected-item="UpdateBOM_NOReceive"
...
export default {
  ...
  mounted() {
    document.title = "SETUP_ROUTE_BOM";
    this.CheckPrivilege();
    //this.GetCustomer();
  },
  methods: {    

    UpdateMO_NUMBER:function(event) {
      this.model.MO_NUMBER = event.target.value;      
      this.GetRoute_Bom(event.target.value);
    },
    UpdateMO_NUMBERReceive(value) {     
      this.model.MO_NUMBER = value;
      this.GetRoute_Bom(value);
    }, 
    UpdateROUTE_NAMEReceive(value) {     
      this.model.ROUTE_NAME = value;
    },   
    UpdateBOM_NOReceive(value) {     
      this.model.BOM_NO = value;
    },   
    async GetRoute_Bom(value) {
      var payload = {
        database_name: localStorage.databaseName,
        MO_NUMBER:value,
      };
      var { data } = await Repository.getRepo("GetAllRoute_Bom", payload);
      this.ListROUTE_NAME = [];
      data.data.forEach((element) => {
        this.ListROUTE_NAME.push(element.ROUTE_NAME);
      });
      this.ListBOM_NO = [];
      data.data1.forEach((element) => {
        this.ListBOM_NO.push(element.BOM_NO);
      });
    },
    ...
    async SaveData() {
      if (this.model.MO_NUMBER == "" && (this.model.ROUTE_NAME == "" || this.model.BOM_NO == "" )) {        
    ...
    DeleteRecord(item) {
      ...
      this.$swal({
        ...
        var { data } = await Repository.getRepo("DeleteRoute_Bom", payload);
        if (data.result == "ok") {
          await this.QuerySearch();
          if (localStorage.language == "En") {
            this.$swal("", "Apply successfully", "success");
    ...
    ShowDetail(detail) {
      ...
      //this.UpdateMO_NUMBER(detail.MO_NUMBER);
      this.isDisabled = true;
      this.GetRoute_Bom(detail.MO_NUMBER);
    ...
    async LoadComponent() {
      ...
      var { data } = await Repository.getRepo("Setup_Route_Bom", payload);
      this.DataTable = [];
