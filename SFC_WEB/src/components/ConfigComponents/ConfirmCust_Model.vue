  ...
  <div class="div-all">
        ...
        <template v-if="DataTableHeader">
          <table id="tableMain" class="table mytable">
            ...
            <template v-for="(item, index) in DataTable" :key="index">
              <tr>     
                <template v-if="item.STATUS != 'CONFIRM'"     >     
    ...
    <div class="div-bellow">
      <div class="form-row">
        ...
        <!-- <div class="col-md-4 mb-4">
          <label for="DATA6">CONFIRM_CUST_MODEL</label>
          <input
              type="checkbox"
              class="form-control text-element"
              id="validationOrderByBox"
              v-model="model.IS_CONFIRM"
              required>
        </div> -->
        
        <div class="col-md-4 mb-4">
          <label for="validationDefault02">CONFIRM_CUST_MODEL</label>
          <DropdownSearch
...
export default {
  ...
  mounted() {
    document.title = "ConfirmCustModel";
    this.CheckPrivilege();
    this.GetModel();
  },
  methods: {    
    ...
    async SaveData() {
      ...
      if (this.model.IS_CONFIRM == "" || this.model.ID == "") {
        if (localStorage.language == "En") {
          this.$swal("", "Empty fields confirm", "error");
        } else {
          this.$swal("", "Không được bỏ trống confirm", "error");
        }
      } else {
        var titleValue = "";
        var textValue = "";
        ...
        this.$swal({
          ...
          var payload = {
            ...
            CUSTMODEL_DESC10 : this.model.STATUS,
          };

          var { data } = await Repository.getRepo("ConfirmConfig40",payload);
    ...
    ShowDetail(detail) {
        ...
        this.model.STATUS = detail.STATUS == "CONFIRM_LABEL_CUSTMODEL"?"0": detail.STATUS == "CONFIRM"? "1" :
                         detail.STATUS == "CONFIRM_LABEL2_CUSTMODEL" ?"2" : detail.STATUS == "CONFIRM_ME_CUSTMODEL"?"3"  :
                         detail.STATUS == "CONFIRM_PQE_CUSTMODEL"?"4" : "";  
    },
    BackToParent() {
    ...
    async LoadComponent() {
      ...
      var { data } = await Repository.getRepo("GetConfirmConfig40Content", payload);
      this.DataTable = [];
