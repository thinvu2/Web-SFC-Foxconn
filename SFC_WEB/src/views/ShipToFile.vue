<template>
  <div class="content_main">
        ...
        <table class="col-9">
          <tr class="row" style="margin-bottom: 5px; margin-left: 2px">
            <td class="col-3">
              <div class="row">
                <div class="col-3">
                  <b class="titleType">Serial Number</b>
                </div>                
                <div class="col-9">
                  <textarea
             ...
             <td class="col-4">
              <div class="row">
                <div class="col-3">
                  <b class="titleType">License_No</b>
                </div>
                <div class="col-9">
                  <textarea
                    v-model="textListSearch3"
                    class="form-control col-sm-15"
                ></textarea>
                </div>
              </div>
            ...
            <td class="col-4">
              <div class="row">
                <div class="col-3">
                  <b class="titleType">Mcarton_no</b>
                </div>
                <div class="col-9">
                  <textarea
                    v-model="textListSearch1"
                    class="form-control col-sm-15"
                ></textarea>
                </div>
              </div>
            ...
            <td class="col-4">
              <div class="row">
                <div class="col-8">
                  <input type="checkbox" id="checkR117" v-model="IscheckR117" style=" margin-right: 8px;">
                  <label class="titleType" for="checkR117"> Detail</label>
                </div>
              </div>
            </td>
            <td class="col-4">
    ...
    <!--  <div class="col-3">
        <p class="text-total">
          Total = <span class="text-total-count"> {{ moQTY }} </span>
        </p>
      </div>
      <div class="col-3">
        <p class="text-total">
  ...
  methods: {      
    ...
    async queryShiptoFile() {
        ...
        if(this.textListSearch != null){
          ...
          listSerial.push({ input: element.trim().replace(/[\u200B-\u200D\uFEFF]/g,'') });
          });
        }else{
          listSerial = null;
        }

        if(this.textListSearch3 != null){
          this.textListSearch3.split(/\n/).forEach((element) => {
          if (
            element.trim().length > 20 &&
            element.trim().indexOf(":") !== -1
          ) {
          ...
          if (texter3 != "") {
            this.textListSearch3 = texter3;
          }
          listLicense.push({ input: element.trim().replace(/[\u200B-\u200D\uFEFF]/g,'') });
          });
        }else{
          listLicense = null;
        }
        ...
        if(this.textListSearch1 != null){
          this.textListSearch1.split(/\n/).forEach((element) => {
          if (
            element.trim().length > 20 &&
            element.trim().indexOf(":") !== -1
          ) {
          ...
          if (texter1 != "") {
            this.textListSearch1 = texter1;
          }
          listcarton.push({ input: element.trim().replace(/[\u200B-\u200D\uFEFF]/g,'') });
          });
        }else{
          listcarton = null;
        }
        ...
        if (listSerial == listLicense == listcarton == null){
          return false;
        }
        ...
        var payload = {
          ...
          license_no: listLicense,
          mcarton_no: listcarton,
          ischeckR117: this.IscheckR117
        };

        var { data } = await Repository.getRepo("GetDataShipToFile", payload);
        this.motypeShow = false;
        this.moShow = false;
        this.modelShow = false;
        this.sectionShow = false;
