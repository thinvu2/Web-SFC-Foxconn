    ...
    public class Config74Controller : ApiController
    {
        ...
        public async Task<HttpResponseMessage> GetConfig74Content(Config74Element model)
        {
         ...
         "       and EMP_NO not like 'AUTO%' "; 
            ...
            {
                getdataemp = $"select ROWIDTOCHAR(ROWID) ID,A.MODEL_NAME,A.GROUP_NAME,A.TYPE,A.CONDITION,TO_CHAR(A.CURRENT_DATA) CURRENT_DATA ,A.STATUS,ERROR_CODE from sfis1.c_ete_config_t A where A.model_name in (select model_name from SFIS1.C_MODEL_DESC_T) order by A.model_name,A.group_name,A.type";
                detaildata = $"select * from ({detaildata}) where rownum < 30";
            }
            else
            {
                getdataemp = $"select ROWIDTOCHAR(ROWID) ID,A.MODEL_NAME,A.GROUP_NAME,A.TYPE,A.CONDITION ,TO_CHAR(A.CURRENT_DATA) CURRENT_DATA,A.STATUS,ERROR_CODE from sfis1.c_ete_config_t A where A.model_name in (select model_name from SFIS1.C_MODEL_DESC_T where model_name LIKE UPPER('{model.MODEL_NAME}%') ) order by A.model_name,A.group_name,A.type";
        ...
        public async Task<HttpResponseMessage> GetTypeConfig74(Config74Element model)
        {
            ...
            DataTable dt = new DataTable();
            getdatatype = $"  select distinct type from SFIS1.C_ETE_CONFIG_T     ";
